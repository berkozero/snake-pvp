...
describe('engine', () => {
  ...
  it('keeps the legacy engine facade wired to replay frame helpers', () => {
    ...
    expect(frames[0]?.reason).toBe('initial');
    expect(frames.some((frame) => frame.reason === 'action_applied')).toBe(true);
    expect(frames.at(-1)?.snapshot.clockMs).toBe(2_525);
  });

  it('serializes engine state through the shared simulator snapshot shape', () => {
    const state = makePlayingState({
      clockMs: 1_250,
      players: {
        p1: {
    ...
    expect(snapshot.clockMs).toBe(1_250);
    expect(snapshot.players.p1.head).toEqual({ x: 4, y: 5 });
    expect(snapshot.players.p1.segments).toEqual(state.players.p1.segments);
    expect(snapshot.players.p1.pendingDirection).toBe('down');
    expect(snapshot.players.p1.length).toBe(4);
  });
