...
import { getRespawnRemainingMs, pickFoodCell, queueDirection, startCountdown, tick } from '../src/game/engine';
import type { Direction, PlayerId, RoundState } from '../src/game/types';
import {
  PROTOCOL_VERSION,
  ROOM_ID,
...
export class MainRoom {
  ...
  readonly slots: Record<PlayerId, SlotState> = {
    p1: { ownerSessionId: null, name: null, connected: false, reservedUntil: null },
    p2: { ownerSessionId: null, name: null, connected: false, reservedUntil: null },
  };
  ...
  snapshotFor(socketId: string | null): RoomSnapshotMessage {
    ...
    const slots: Record<PlayerId, SlotSnapshot> = {
      p1: {
        claimed: this.slots.p1.ownerSessionId !== null,
        name: this.slots.p1.name,
        connected: this.slots.p1.connected,
      },
      p2: {
        claimed: this.slots.p2.ownerSessionId !== null,
  ...
  private handleJoin(
    ...
    if (slotState.ownerSessionId !== null) {
      this.rejectJoin(
        session,
        message.requestId,
  ...
  private handleInput(session: SessionRecord, direction: Direction, inputSeq: number): void {
    ...
    const nextState = queueDirection(this.game, this.directionKeyFor(player.id, direction));
    const accepted = nextState.players[player.id].pendingDirection === direction;
    if (!accepted) {
      this.rejectAction(session, null, 'invalid_direction');
      return;
    }
  ...
  private directionKeyFor(slot: PlayerId, direction: Direction): string {
    const keys: Record<PlayerId, Record<Direction, string>> = {
      p1: {
        up: 'w',
        down: 's',
        left: 'a',
  ...
  private refreshLobbyPhase(): void {
    ...
    const claimed = SLOT_IDS.filter((slot) => this.slots[slot].ownerSessionId !== null);
